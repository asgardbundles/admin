...
class GeneratorHooks extends \Asgard\Hook\HooksContainer {
	...
	**/
	public static function bundle($chain, $bundle) {
		foreach($bundle['entities'] as $name=>$entity) {
			echo "
			\Asgard\Admin\Libs\AdminMenu::instance()->menu[0]['childs'][] = array('label' => '".ucfirst($entity['meta']['label_plural'])."', 'link' => '".$entity['meta']['plural']."'); 
			\Asgard\Admin\Libs\AdminMenu::instance()->home[] = array('img'=>\Asgard\Core\App::get('url')->to('".$bundle['name']."/".$entity['meta']['plural'].".svg'), 'link'=>'".$entity['meta']['plural']."', 'title' => __('".ucfirst($entity['meta']['label_plural'])."'), 'description' => __('')); ";
		}
	}
}